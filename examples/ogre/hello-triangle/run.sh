#!/usr/bin/env bash
# Build the hello-triangle example and run it.
#
...
if [ ! -f "$adapter" ]; then
    echo "hello-triangle: building the OGRE adapter (tension-ogre/build.sh)..."
...
if [ ! -x node_modules/.bin/asc ]; then
    echo "hello-triangle: installing the guest's build dependencies (npm install)..."
    npm install --silent
fi
