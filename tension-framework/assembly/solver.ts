...
//
// What this file does NOT declare: the guest's own three exports
// (`_derivative`, `deriv_buf_in`, `deriv_buf_out`). Under `source: "wasm"`
// the host resolves those from the guest module and performs the binding
// itself (GUEST_ABI.md §3.6) — the guest exports them and calls nothing to
// register them.
//
...
export class Solver {
  ...
  /**
   * Create a solver from a JSON config. Returns `null` on any failure:
   * malformed config, unknown method, unmet source requirement, a method or
   * source not available in this build, allocation failure, or a full id
   * table.
   */
  static create(configJson: string): Solver | null {
    const bytes = String.UTF8.encode(configJson);
    const id = hostSolverCreate(changetype<usize>(bytes), i32(bytes.byteLength));
    if (id < 1) return null;
