# Tension TypeScript WebAssembly Demo

This project demonstrates how to use the Tension engine with TypeScript and compile it to WebAssembly. It provides a basic setup for creating a 3D demo environment using the Tension engine.

## Project Structure

```
tension-ts-wasm-demo
├── src
│   ├── index.ts            # Main entry point for the TypeScript application
│   ├── wasm-loader.ts      # Handles loading the WebAssembly module
│   └── types
│       └── tension.d.ts # Type definitions for the Tension engine
├── assembly
...
The demo showcases the capabilities of the Tension engine, including rendering 3D entities and handling user interactions. You can modify the source files in the `src` directory to customize the demo or add new features.
