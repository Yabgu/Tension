...
impl SdlRenderer {
    pub fn new(config: &EngineConfig) -> Result<Self> {
        ...
        let window = video_subsystem
            .window("Frankenstein Engine - WASM Demo", config.window_width, config.window_height)
            .position_centered()
            .opengl()
            .build()
...
impl frankenstein_core::RendererTrait for SdlRenderer {
    fn begin_frame(&mut self) -> frankenstein_core::Result<()> {
        self.begin_frame()
    }
    
    fn render_world(&mut self, world: &World, alpha: f64) -> frankenstein_core::Result<()> {
        self.render_world(world, alpha)
    }
    
    fn end_frame(&mut self) -> frankenstein_core::Result<()> {
        self.end_frame()
    }
    
    fn handle_events(&mut self, input_manager: &mut InputManager) -> frankenstein_core::Result<()> {
