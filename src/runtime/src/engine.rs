...
impl WasmEngine {
                        ...
                        if let Ok(mat) = std::str::from_utf8(&data[start..end]) {
                            ...
                            unsafe {
                                if let Some(world_ptr_usize) = state.world_ptr {
                                    let world_ptr = world_ptr_usize as *mut tension_core::World;
    ...
    pub fn attach_world_ptr(&mut self, world_ptr: *mut tension_core::World) {
        let mut state = self.host_state.lock().unwrap();
        state.world_ptr = Some(world_ptr as usize);
    }

    /// Trait-compatible attach_world wrapper
    pub fn attach_world(&mut self, world_ptr: *mut tension_core::World) -> anyhow::Result<()> {
...
impl tension_core::WasmRuntimeTrait for WasmEngine {
    ...
    fn reload(&mut self) -> Result<()> {
        HotReloadable::reload(self).map_err(|e| tension_core::EngineError::Runtime(e.to_string()))
    }
    ...
    fn attach_world(&mut self, world_ptr: *mut tension_core::World) -> anyhow::Result<()> {
