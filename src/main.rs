...
use tension_core::{Engine, EngineConfig};
use tension_runtime::{WasmEngine, RuntimeConfig};
...
fn main() -> anyhow::Result<()> {
    ...
    tracing_subscriber::registry()
        .with(
                ...
                .unwrap_or_else(|_| "tension=debug,info".into()),
        )
        .with(tracing_subscriber::fmt::layer())
        .init();

    tracing::info!("🎮 Tension Engine - WASM-Driven Game Engine");
...
fn run_main_loop(mut engine: Engine) -> anyhow::Result<()> {
    ...
    use tension_core::api::{InputEvent, InputModifiers, MouseButton};
...
fn load_demo_modules(engine: &mut Engine, demo_name: &str) -> anyhow::Result<()> {
    ...
    let world_ptr: *mut tension_core::World = engine.world_mut() as *mut tension_core::World;
    ...
    let mut loader = tension_runtime::ModuleLoader::new(module_dir.clone());
