...
int32_t tension_solver_create(const char *config_json, size_t config_len)
{
    ...
    if (builtin >= 0 && builtin != TS_EULER_INDEX)
    ...
    if (!h->is_plugin) {
        h->ws_slots = tension_solver_euler_workspace_size(h->dim);
        if (h->ws_slots < 1) {
            free(h->y);
            h->y = NULL;
            return TS_EINVAL;
...
int32_t tension_solver_step(int32_t id, double dt)
{
    ...
    if (h->builtin != TS_EULER_INDEX)
        return TS_ENOSYS; /* unreachable: create refuses other built-ins */

    h->stepped = 1;
    rc = tension_solver_euler_step(h->y, h->dim, h->t, dt, h->ws,
                                   h->bound_derivative, NULL, NULL, &status);
    if (rc == 0)
        h->t += dt;
    return rc;
}
